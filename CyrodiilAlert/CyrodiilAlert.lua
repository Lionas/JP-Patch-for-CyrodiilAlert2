...
--
--function CA.initFonts()
--	
--	if (not LMP) then return end
--
--	local fontArtList = {
--		["GenEiGothicPR"] = "CyrodiilAlert/fonts/GenEiGothicP-Regular.otf",
--	}
--
--	-- Register custom fonts
--	for fontName, fontPath in pairs(fontArtList)
--	do 
...
--		CA.initFonts()
--		local fontCommonSettings = LMP:Fetch('font', "GenEiGothicPR") .. "|16|soft-shadow-thin"
...
--	d("CA Debug: Owner Changed, BGContext " .. battlegroundContext)
	...
	if ((CA.vars.inside == false) and (IsInImperialCity()) and (keepId < 140)) then
		do return end
	end
	...
	if (CA.vars.showTowns == false) and (keepId == (149 or 151 or 152)) then
		do return end
	end

	if (CA.vars.showDistricts == false) and (keepId == (141 or 142 or 143 or 146 or 147 or 148)) then
		do return end
	end

	if (CA.vars.showDistrictsOut == false) and (keepId == (141 or 142 or 143 or 146 or 147 or 148)) and (not IsInImperialCity()) then
		do return end
	end
	...
	if (keepId > 140) and (keepId < 149) then
...
--	d("CA Debug: Attack Status Changed, BGContext " .. battlegroundContext)
	...
	if (underAttack) and (CA.vars.showAttack) then
		if (CA.vars.chatOutput) then
		d(uaText)
			if (allSiege ~= 0) and (keepId < 140) then
				if (CA.vars.siegesAttDef) and (CA.vars.siegesByAlliance) then
					d(siegesTextAll)
				elseif (CA.vars.siegesAttDef) and (not CA.vars.siegesByAlliance) then
					d(siegesTextAttDef)
			...
			elseif (allSiege == 0) and (keepId < 140) and ((CA.vars.siegesAttDef) or (CA.vars.siegesByAlliance)) then
				d(siegesTextNone)
			end
		end
		...
		if (not CA.vars.onlyChat) then
			if (not CA.vars.siegesAttDef) and (not CA.vars.siegesByAlliance) then
				...
				elseif (allSiege == 0) and (keepId < 140) then
...
--	d("Control State: " .. ctrlState[controlState])
	...
	if (CA.vars.showFlagsResources == false) and (((keepId > 21) and (keepId < 28)) or ((keepId > 33) and (keepId < 88))) then
--		d("CA Debug: Objective Control State, ID " .. objectiveId .. ": " .. objectiveName .. ", identified as Resource")
		do return end
	end
	if (CA.vars.showFlagsTowns == false) and (keepId == (149 or 151 or 152)) then
--		d("CA Debug: Objective Control State, ID " .. objectiveId .. ": " .. objectiveName .. ", identified as Town")
		do return end
	end
	if (CA.vars.showFlagsDistricts == false) and (keepId == (141 or 142 or 143 or 146 or 147 or 148)) then
